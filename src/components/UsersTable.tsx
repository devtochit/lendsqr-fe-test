...
const UsersTable = ({ tableData }: Props) => {
    ...
    Listener(promptModalRef, clickOutsidehandler);
    ...
    const renderUsers = currentUsers?.map((user: any, i: number) => (
        ...
        >
            ...
            <td>{
                (getYearsBetween(user.createdAt, user.lastActiveDate)) <= 40 ? <span className="status__btn active-status">{'Active'}</span>
                    : (getYearsBetween(user.createdAt, user.lastActiveDate)) > 40 && (getYearsBetween(user.createdAt, user.lastActiveDate)) <= 60 ? <span className="status__btn pending-status">{'Pending'}</span>
                        : (getYearsBetween(user.createdAt, user.lastActiveDate)) > 60 && (getYearsBetween(user.createdAt, user.lastActiveDate)) <= 90 ? <span className="status__btn inactive-status">{'Inactive'}</span>
                            : (getYearsBetween(user.createdAt, user.lastActiveDate)) > 90 && <span className="status__btn blacklisted-status">{'Blacklisted'}</span>
            }
            </td>
            <td onClick={() => setActiveIndex(i)} className='action__group'><Ellipsis />
                {activeIndex === i &&
                    <motion.div ref={promptModalRef}
                        ...
                        <div><BlackListUserIcon /> Blacklist User</div>
